<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900 p-4">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-6">
      <div v-if="cargando" class="text-center py-8 text-slate-400">
        <p>Cargando...</p>
      </div>
      ...
      <template v-else-if="!logueado">
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          ...
          <button
            type="submit"
            class="w-full py-4 bg-blue-600 text-white text-lg font-semibold rounded-xl hover:bg-blue-700 transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50 focus:ring-4 focus:ring-blue-200"
            :disabled="loading"
          >
...
async function handleLogin() {
  ...
  loading.value = true
  try {
    const res = await axios.post(`/api/c/${route.params.token}/login`, { dni: dni.value })
